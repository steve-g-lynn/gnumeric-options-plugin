...
#define DEF_ARG_CC_OPT { GNM_FUNC_HELP_ARG, F_("cost_of_carry:net cost of holding the underlying asset (for common stocks, the risk free rate less the dividend yield), defaults to the risk-free rate") }
...
/* Black-Scholes on a share. Changed by Stephen G Lynn to set default
cost-of-carry b equal to r (not zero) */
...
opt_bs (GnmFuncEvalInfo *ei, GnmValue const * const *argv)
{
	...
	gnm_float b = argv[6] ? value_get_as_float (argv[6]) : value_get_as_float (argv[4]);
...
static GnmFuncHelp const help_opt_bs[] = {
	...
	DEF_ARG_VOLATILITY,
	DEF_ARG_CC_OPT,
	{ GNM_FUNC_HELP_DESCRIPTION, F_("OPT_BS uses the generalized Black-Scholes model to calculate "
					"the price of a European option struck at @{strike} "
					"on an asset with spot price @{spot}."
					"This function is modified by Stephen G Lynn to set the optional @{cost_of_carry} variable to @{rate} rather than 0.")},
	DEF_NOTE_UNITS,
...
/* Black-Scholes on a call 
 opt_bs_call (s,x,t,r,s)
*/
static GnmValue *
opt_bs_call (GnmFuncEvalInfo *ei, GnmValue const * const *argv)
{
	...
	gnm_float gfresult = opt_bs1 (OS_Call, s, x, t, r, v, r);
...
static GnmFuncHelp const help_opt_bs_call[] = {
	{ GNM_FUNC_HELP_NAME, F_("OPT_BS_CALL:price of a European call")},
	DEF_ARG_SPOT,
	DEF_ARG_STRIKE,
	DEF_ARG_TIME_MATURITY_Y,
	DEF_ARG_RATE_RISKFREE,
	DEF_ARG_VOLATILITY,
	{ GNM_FUNC_HELP_DESCRIPTION, F_("OPT_BS_CALL uses the Black-Scholes model to calculate "
					"the price of a European call option struck at @{strike} "
					"on an asset with spot price @{spot}.")},
	DEF_NOTE_UNITS,
        { GNM_FUNC_HELP_SEEALSO, "OPT_BS_DELTA,OPT_BS_RHO,OPT_BS_THETA,OPT_BS_VEGA,OPT_BS_GAMMA"},
        { GNM_FUNC_HELP_END}
...
/* Black-Scholes on a put 
 opt_bs_put (s,x,t,r,s)
*/
static GnmValue *
opt_bs_put (GnmFuncEvalInfo *ei, GnmValue const * const *argv)
{
	...
	gnm_float gfresult = opt_bs1 (OS_Put, s, x, t, r, v, r);
...
static GnmFuncHelp const help_opt_bs_put[] = {
	{ GNM_FUNC_HELP_NAME, F_("OPT_BS_PUT:price of a European put")},
	DEF_ARG_SPOT,
	DEF_ARG_STRIKE,
	DEF_ARG_TIME_MATURITY_Y,
	DEF_ARG_RATE_RISKFREE,
	DEF_ARG_VOLATILITY,
	{ GNM_FUNC_HELP_DESCRIPTION, F_("OPT_BS_PUT uses the Black-Scholes model to calculate "
					"the price of a European put option struck at @{strike} "
					"on an asset with spot price @{spot}.")},
	DEF_NOTE_UNITS,
        { GNM_FUNC_HELP_SEEALSO, "OPT_BS_DELTA,OPT_BS_RHO,OPT_BS_THETA,OPT_BS_VEGA,OPT_BS_GAMMA"},
        { GNM_FUNC_HELP_END}
...
opt_bs_gamma (GnmFuncEvalInfo *ei, GnmValue const * const *argv)
{
	...
	gnm_float r = value_get_as_float (argv[3]);
	gnm_float v = value_get_as_float (argv[4]);
	gnm_float b = argv[5] ? value_get_as_float (argv[5]) : value_get_as_float (argv[3]);
...
static GnmFuncHelp const help_opt_binomial_crr[] = {
	{ GNM_FUNC_HELP_NAME, F_("OPT_BINOMIAL_CRR:theoretical price of either an American or European style option using a Cox-Ross-Rubinstein binomial tree model")},
...
static GnmFuncHelp const help_opt_binomial_eqprob[] = {
	{ GNM_FUNC_HELP_NAME, F_("OPT_BINOMIAL_EQPROB:theoretical price of either an American or European style option using an Equal Probabilities binomial tree model")},
        { GNM_FUNC_HELP_ARG, F_("amer_euro_flag:'a' for an American style option or 'e' for a European style option")},
	DEF_ARG_CALL_PUT_FLAG,
        { GNM_FUNC_HELP_ARG, F_("num_time_steps:number of time steps used in the valuation")},
	DEF_ARG_SPOT,
...
GnmFuncDescriptor const derivatives_functions [] = {
	...
	{ "opt_bs_call",
	  "fffff",
	  help_opt_bs_call, opt_bs_call, NULL,
	  GNM_FUNC_SIMPLE, GNM_FUNC_IMPL_STATUS_UNIQUE_TO_GNUMERIC, GNM_FUNC_TEST_STATUS_BASIC },
	  
	{ "opt_bs_put",
	  "fffff",
	  help_opt_bs_put, opt_bs_put, NULL,
	  GNM_FUNC_SIMPLE, GNM_FUNC_IMPL_STATUS_UNIQUE_TO_GNUMERIC, GNM_FUNC_TEST_STATUS_BASIC },
	...
	{ "opt_binomial_crr",
	  "ssffffff|f",
	  help_opt_binomial_crr, opt_binomial_crr, NULL,
	  GNM_FUNC_SIMPLE, GNM_FUNC_IMPL_STATUS_UNIQUE_TO_GNUMERIC, GNM_FUNC_TEST_STATUS_BASIC },
	  
	{ "opt_binomial_eqprob",
	  "ssffffff|f",
	  help_opt_binomial_eqprob, opt_binomial_eqprob, NULL,
	  GNM_FUNC_SIMPLE, GNM_FUNC_IMPL_STATUS_UNIQUE_TO_GNUMERIC, GNM_FUNC_TEST_STATUS_BASIC },
